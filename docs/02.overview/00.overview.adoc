...
The figure below, identifies the main workflows on which the Login Service participates: //TODO
...
=== UMA workflow
By default, some actions are protected by a UMA (User-Managed access) workflow, that is already described in the Master System Design document (ref. [PENDING]). This can be best described by the following diagram:
TODO: import UMA sequence diagram
